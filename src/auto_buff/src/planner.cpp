...
BuffPlanner::BuffPlanner(quill::Logger *logger)
    : logger_(logger), solver_(logger_, makeBallisticConfig()),
  ...
  Eigen::MatrixXd x_max =
      Eigen::MatrixXd::Constant(2, trajectory_horizon_, 1e17);
  Eigen::MatrixXd u_min = Eigen::MatrixXd::Constant(1, trajectory_horizon_ - 1,
                                                    -par::TRACKER_MAX_YAW_ACC);
  Eigen::MatrixXd u_max = Eigen::MatrixXd::Constant(1, trajectory_horizon_ - 1,
                                                    par::TRACKER_MAX_YAW_ACC);
  ...
  tiny_setup(&pitch_solver_, A, B, f, Q_pitch.asDiagonal(),
             R_pitch.asDiagonal(), 1.0, 2, 1, trajectory_horizon_, 0);
  u_min = Eigen::MatrixXd::Constant(1, trajectory_horizon_ - 1,
                                    -par::TRACKER_MAX_PITCH_ACC);
  u_max = Eigen::MatrixXd::Constant(1, trajectory_horizon_ - 1,
                                    par::TRACKER_MAX_PITCH_ACC);
...
msgs::AimCommand BuffPlanner::plan(const BuffTargetSnapshot &snapshot,
  ...
  try {
    ...
    LOG_WARNING(logger_, "buff planner failed to build reference: {}",
                e.what());
    return {.control = false};
  }
  ...
  const int fire_index = std::clamp(output_index + par::TRACKER_SHOOT_OFFSET, 0,
                                    trajectory_horizon_ - 1);
  const int acc_index = std::min(output_index, trajectory_horizon_ - 2);
  const double fire_error = std::hypot(
      tools::shortestRadianDistance(yaw_solver_->work->x(0, fire_index),
                                    reference.state_reference(0, fire_index)),
      pitch_solver_->work->x(0, fire_index) -
          reference.state_reference(2, fire_index));

  msgs::AimCommand cmd{};
  cmd.control = true;
  // TODO
  // cmd.fire = fire_error < par::TRACKER_FIRE_THRESH_RAD;
  cmd.target_yaw = static_cast<float>(
      tools::limitRadian(reference.state_reference(0, output_index)));
  cmd.target_pitch =
      static_cast<float>(reference.state_reference(2, output_index));
  cmd.yaw = static_cast<float>(
      tools::limitRadian(yaw_solver_->work->x(0, output_index)));
...
std::optional<BuffAimTrack>
BuffPlanner::selectTarget(const BuffTargetSnapshot &snapshot) {
  ...
  for (const auto &[track_index, group] : groups) {
    const auto current =
        group.observed.has_value() ? group.observed : group.predicted;
...
BuffTrajectoryReference
BuffPlanner::buildTrajectoryReference(const BuffAimTrack &track,
                                      const msgs::GimbalInfo &gimbal_info,
  ...
  const double reference_dt =
      resolveReferenceDt(track, current_solution.value());
