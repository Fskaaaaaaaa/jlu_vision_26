...
CameraCalibration makeCalibrationFromSharedConfig(
    ...
    calibration.image_size = cv::Size(config.view_width_px, config.view_height_px);
    ...
    std::memcpy(calibration.camera_matrix.data, config.camera_matrix.data(),
                calibration.camera_matrix.elemSize() * config.camera_matrix.size());
    std::memcpy(calibration.dist_coeffs.data, config.distortion_coefficients.data(),
                calibration.dist_coeffs.elemSize() * config.distortion_coefficients.size());
    return calibration;
}

CameraCalibration makeCalibrationFromMessage(const msgs::CameraInfo& camera_info_msg) {
    ...
    std::memcpy(calibration.camera_matrix.data, camera_info_msg.camera_matrix.data(),
...
CameraCalibration waitForCameraCalibration() {
    ...
    std::cout << "等待相机标定通道: "
              << par::CAMERA_INFO_SERVICE << "/" << par::CAMERA_INFO_INSTANCE
    ...
    while (!iox::posix::hasTerminationRequested()) {
        ...
        camera_info_sub.take()
            .and_then([&](const iox::popo::Sample<const msgs::CameraInfo, const msgs::Header>& sample) {
                got_message = true;
...
void updateTaskMode(quill::Logger* logger,
    ...
    task_mode_sub.take()
        .and_then([&](const iox::popo::Sample<const msgs::TaskMode, const msgs::Header>& sample) {
            current_mode = static_cast<types::TaskMode>(sample->mode);
        })
        .or_else([&](auto) {});
    while (task_mode_sub.take().and_then(
        [&](const iox::popo::Sample<const msgs::TaskMode, const msgs::Header>& sample) {
            current_mode = static_cast<types::TaskMode>(sample->mode);
        })) {
    }
...
int main() {
    ...
    auto* logger =
        tools::initAndGetLogger(APP_NAME, quill::LogLevel::Info, "logs/buff_detector");

    std::cout << "发布结果通道: "
              << par::RESULT_SERVICE << "/" << par::RESULT_INSTANCE << "/"
              << par::RESULT_EVENT << "\n";
    ...
    if (par::FRAME_SOURCE == par::FrameSource::SubscribedCamera) {
        std::cout << "订阅工业相机通道: image_raw/" << par::CAMERA_NAME << "/data\n";
    } else {
        std::cout << "视频路径: " << par::VIDEO_PATH << "\n";
    }

    const double final_output_fps = par::OUTPUT_FPS;
    const int wait_time = final_output_fps > 1e-6
        ? static_cast<int>(std::round(1000.0 / (final_output_fps * par::PLAYBACK_SPEED)))
        : 1;
    ...
    std::optional<iox::popo::Subscriber<msgs::TaskMode, msgs::Header>> task_mode_sub;
    std::optional<hardware::CameraParamsChanger> cam_params_changer;
    if (par::FRAME_SOURCE == par::FrameSource::SubscribedCamera) {
        task_mode_sub.emplace(
            make_service_desc(
                par::TASK_MODE_SERVICE,
    ...
    const auto process_frame =
        [&](const cv::Mat& frame, const std::string& source_frame_id,
        ...
        const bool ok =
            detector.processFrame(frame, local_frame_id, timestamp_sec, vis, result_pkt);
        if (!ok) {
            return true;
        }
        ...
        if (!video_writer_inited && par::SAVE_VIDEO) {
            ...
            video_writer.open(par::OUTPUT_VIDEO, fourcc, final_output_fps, vis.size());
    ...
    try {
        if (par::FRAME_SOURCE == par::FrameSource::SubscribedCamera) {
            ...
            while (!iox::posix::hasTerminationRequested()) {
                ...
                image_sub.take()
                    .and_then([&](const iox::popo::Sample<const msgs::Image1440x1080_8UC3,
                                                          const msgs::Header>& sample) {
                        got_frame = true;
                        if (!isBuffTaskMode(current_task_mode)) {
                            return;
                        }
                        const cv::Mat frame(sample->rows, sample->cols, sample->cv_type,
                                            const_cast<unsigned char*>(sample->data));
                        const std::string frame_id = sample.getUserHeader().frame_id.c_str();
                        should_continue = process_frame(
                            frame,
                            frame_id.empty() ? std::string{par::CAMERA_FRAME_ID} : frame_id,
