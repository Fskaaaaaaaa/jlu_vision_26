...
void appendTarget(std::vector<BuffTrackedTarget>& targets, bool valid,
                  int track_index, bool predicted, int model_type, double fly_time,
                  float x, float y, float z, const Eigen::Isometry3d& odom_from_source,
    ...
    target.position_odom = transformPoint(odom_from_source, target.position_source);
...
bool isHeartbeatPacket(const TargetCoordsPacket& packet) {
    return packet.heart_beat > 0 ||
           (packet.observed_target_count == 0 &&
...
BuffTrackerNode::BuffTrackerNode(quill::Logger* logger)
    : logger_(logger),
      task_mode_sub_(make_service_desc(par::TASK_MODE_SERVICE, par::TASK_MODE_INSTANCE,
                                       par::TASK_MODE_EVENT)),
...
void BuffTrackerNode::pollGimbalInfo() {
    gimbal_info_sub_.take()
        .and_then([&](const iox::popo::Sample<const msgs::GimbalInfo, const msgs::Header>& sample) {
            latest_gimbal_info_ = *sample;
        })
        .or_else([&](auto) {});
    while (gimbal_info_sub_.take().and_then(
        [&](const iox::popo::Sample<const msgs::GimbalInfo, const msgs::Header>& sample) {
            latest_gimbal_info_ = *sample;
        })) {
    }
}
    ...
    aimcommand_pub_.loan()
        .and_then([&](iox::popo::Sample<msgs::AimCommand, msgs::Header>& sample) {
            sample.getUserHeader().frame_id = {
                iox::cxx::TruncateToCapacity, par::ODOM_FRAME_ID,
...
BuffTrackerNode::transformPacket(const TargetCoordsPacket& packet,
    ...
    try {
        const Eigen::Isometry3d odom_from_source = tf_buffer_.get(
            par::ODOM_FRAME_ID, source_frame_id, stamp,
            std::chrono::nanoseconds{
                static_cast<int64_t>(par::TF_QUERY_TOLERANCE_MS * 1e6)});
        const Eigen::Isometry3d gimbal_from_source = tf_buffer_.get(
            par::GIMBAL_FRAME_ID, source_frame_id, stamp,
            std::chrono::nanoseconds{
                static_cast<int64_t>(par::TF_QUERY_TOLERANCE_MS * 1e6)});
...
void BuffTrackerNode::updateSnapshot(std::optional<BuffTargetSnapshot> snapshot) {
    std::scoped_lock lk(snapshot_mtx_);
    latest_snapshot_ = std::move(snapshot);
}

void BuffTrackerNode::logTransformFailure(
    const std::string& source_frame_id, const std::string& error_message) const {
