...
//   return
//       [this](const Vec_n &x, double dt) { return limitAngleX(this->f(x, dt));
//       };
// }
...
public:
  ...
  virtual std::vector<std::function<Eigen::Vector4d(const Vec_n &x, double dt)>>
  getKinematicFuncXyza() = 0;
...
public:
  ...
  OutpostEkf() = delete;
  OutpostEkf(ParamGetter param_getter, const std::string &name = "outpost_ekf",
             std::vector<int> angle_pos_x = {},
             std::vector<int> angle_pos_z = {});
