...
ArmorDrawer::ArmorDrawer(const std::array<double, 9> &camera_matrix,
    ...
    : camera_optical_frame_(name_of_camera_optical_frame), odom_(name_of_odom),
      camera_matrix_(
...
void ArmorDrawer::drawArmor(cv::Mat &img, const Eigen::Vector3d &armor_xyz,
                            ...
                            const cv::Scalar &color, const rclcpp::Time &time,
                            bool draw_big_armor) {
  // auto color = std::fabs(armor_rpy.z()) >= 90.0 ? Color::BLUE : Color::RED;
  // 背面是蓝色，正面是红色
