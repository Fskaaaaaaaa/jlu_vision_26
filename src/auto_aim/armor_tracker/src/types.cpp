...
auto_aim::Armor::Armor(const Eigen::Vector3d &center_pos, double center_yaw,
  ...
  this->index = armor_index;
}

auto_aim::Armor auto_aim::Armor::fromTargetStatus(const TargetStatus &status,
                                                  ArmorIndex armor_index) {
  if (status.type == types::ArmorType::Base)
    return Armor::frmoBase(status.center_position, status.center_yaw,
                           armor_index);
  return status.type == types::ArmorType::Outpost
             ? Armor::fromOutpost(status.center_position, status.center_yaw,
                                  status.radius, status.dz_a, status.dz_b,
                                  armor_index)
             : Armor::fromRobot(status.center_position, status.center_yaw,
                                status.radius_a, status.radius_b, status.dz,
                                armor_index);
}
