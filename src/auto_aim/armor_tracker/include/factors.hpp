...
//          W(k-1)--OmegaFactor--W(k)
//          [下面两个观测因子误差定义为由X+A/BZ推导的装甲板位置和直接观测的差]
...
public:
  RotationFactor(const gtsam::SharedNoiseModel &model, gtsam::Key r_pre,
...
class OmegaFactor : public gtsam::NoiseModelFactorN<double, double> {
  using Base = gtsam::NoiseModelFactorN<double, double>;

public:
  OmegaFactor(const gtsam::SharedNoiseModel &model, gtsam::Key w_pre,
              gtsam::Key w_cur);
...
class ArmorRadiusAFactor
    : public gtsam::NoiseModelFactorN<double, gtsam::Rot2, gtsam::Vector3> {
  using Base = gtsam::NoiseModelFactorN<double, gtsam::Rot2, gtsam::Vector3>;
