...
- update26.1.119：才意识到servername和payloadtype是强相关的，这个规则还得改
- update26.1.24：按理说应该是{消息类型、消息来源、消息目的}的命名规范才比较合适

## 关于防呆设计

- 对于仅传输数据的高频链路：{数据类型， 数据来源（哪个程序），data}
- 对于带有请求语义的冷路径：{请求什么， 请求谁， 为什么请求}
  - 例如，请求imu积分重置：{imu_control,  imu_name,  reset}、
  - 对于无需返回值的请求，发布到请求话题上的payload类型int就够了。按理说定义个专门用来搞请求的payload可能更合适，但是懒awa
