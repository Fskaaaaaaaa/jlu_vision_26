...
同时，各个节点的yaml参数文件必须包含指向core.yaml的键值对。core配置文件最顶层键必须是每个节点初始化PoshRuntime时注册的名称（也是节点的程序名称、文件夹名称），每个顶层键下分为两个值为map类型的键值对，其键分别为subtribe和publish，其值存储的map的键值分别为 由GETNAME反射得到的订阅/发布者的的名称+sub/pub 和 其要订阅/发布的话题的{service, instance, event}，其中要求service是该 发布节点/发布该节点订阅的数据的节点 所在**功能模块的名称**，instance是需要收发的数据的**结构体名称**，event是具体化这个消息的**目的**（debug、result .etc）
